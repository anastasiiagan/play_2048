...
from constants import CP, SPEED
...
class Py2048:
    ...
    def new_number(self, k=1): #generuj nową liczbę którą doda
        free_poss = list(zip(*np.where(self.grid == 0)))
        for pos in random.sample(free_poss, k=k):
            if random.random() < 0.1: #bylo .0
                self.grid[pos] = 4
            else:
                self.grid[pos] = 2

    @staticmethod
    def _get_nums(this): #tworzy nowy wiersz/kolumnę po ruchu w 
        this_n = this[this != 0]
        this_n_sum = []
        skip = False
    ...
    def play_step(self, move): # robi krok
        for i in range(N):
            if move in 'lr':
                this = self.grid[i, :]
            else:
    ...
    def draw_game(self): #todo
        self.screen.fill(CP['background'])

        for i in range(N):
            for j in range(N):
                n = self.grid[i][j]

                rect_x = j * self.W // N + self.box_size
                rect_y = i * self.H // N + self.box_size
                rect_w = self.W // N - 2 * self.box_size
                rect_h = self.H // N - 2 * self.box_size
    ...
    def game_over(self): # sprawdza czy jest krok po którym macierz zmieni sie, tj sprawdza czy koniec gry
